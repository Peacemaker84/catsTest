...
const deleteCat = async function (id, tag) {
  /*
	...
	*/
  let res = await fetch(
    `https://sb-cats.herokuapp.com/api/2/Peacemaker84/delete/${id}`,
    {
      method: "DELETE",
    }
  );
...
const updCat = async function (obj, id) {
  let res = await fetch(
    `https://sb-cats.herokuapp.com/api/2/Peacemaker84/update/${id}`,
    {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
...
const updCard = function (data, id) {
  for (let i = 0; i < cards.lengthl; i++) {
    let card = cards[i];
    if (card[i].dataset.id === id) {
      card.firstElementChild.style.backgroundImage = data.img_link
        ? `url(${data.img_link})`
        : `url(img/cat.png)`;
